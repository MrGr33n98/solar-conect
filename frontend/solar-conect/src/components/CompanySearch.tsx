...
export const CompanySearch: React.FC<CompanySearchProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      ...
      try {
        ...
        setError(err.message || 'Ocorreu um erro ao buscar empresas. Por favor, tente novamente mais tarde.');
        setCompanies([]);
        setTotalPages(0);
        setTotalCompanies(0);
      } finally {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto pb-28">
      <h3 className="text-2xl font-semibold text-center text-gray-800 mb-6 pb-2 border-b-2 border-gray-200">
      ...
      {!isLoading && !error && companies.length === 0 && (
        <p className="text-center text-gray-500 py-8">Nenhuma empresa encontrada com os critérios informados.</p>
      )}
      {!isLoading && !error && companies.length > 0 && (
        <p className="text-sm text-gray-600 mb-4">{totalCompanies} empresa(s) encontrada(s).</p>
      )}
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        {companies.map((company) => (
          <div
            key={company.id}
            className="bg-white p-4 md:p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex items-start gap-4"
          >
            {company.logoUrl ? (
              <img
                src={company.logoUrl}
                alt={`${company.name} logo`}
                className="w-16 h-16 md:w-24 md:h-24 object-contain border border-gray-200 rounded-md flex-shrink-0"
              />
            ) : (
              <div className="w-16 h-16 md:w-24 md:h-24 bg-gray-200 text-gray-400 flex items-center justify-center rounded-md flex-shrink-0 text-xs">
                No Logo
              </div>
            )}
          ...
          <div className="mb-2 sm:mb-0">
            ...
            <ul className="inline-flex list-none pl-2 flex-wrap">
              {companiesToCompare.map((c) => (
                <li key={c.id} className="mr-2 mt-1 sm:mt-0 px-2 py-1 bg-gray-700 rounded text-xs">
                  {c.name}
                </li>
              ))}
            </ul>
